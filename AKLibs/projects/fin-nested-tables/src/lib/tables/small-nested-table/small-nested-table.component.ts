...
@Component({
  selector: 'ak-small-nested-table',
  templateUrl: './small-nested-table.component.html',
  styleUrls: ['./small-nested-table.component.scss']
})
export class SmallNestedTableComponent implements OnChanges {
  ...
  public mediumTableColumn = ['ctrls', 'data1'];
  ...
  onDataUpdate() {
    try {
      ...
      this.sortColumn = this.NTData.details && this.NTData.details.sortColumnName ?
        this.NTData.details.sortColumnName[0].toLocaleLowerCase() + this.NTData.details.sortColumnName.substr(1) : '';
      ...
      // Spliting the Column Array into to additinal column and separete the control cells
      this.tableColumnNames.data1 = [...this.displayedColumns[this.layerLevel - 1]];
